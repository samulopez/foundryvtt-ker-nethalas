...
> extends HandlebarsApplicationMixin(ActorSheetV2)<RenderContext, Configuration, RenderOptions> {
  ...
  static PARTS = {
    header: {
      template: TEMPLATES.minion.header,
    },
    tabs: {
  ...
  override get document(): KerNethalasActor<'minion'> {
    if (!super.document.isMinion()) {
      throw new Error('Actor is not a minion');
    }
